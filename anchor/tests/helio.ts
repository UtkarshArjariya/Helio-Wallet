...
import type { Helio } from '../target/types/helio';
...
describe('helio', () => {
  ...
  async function initialize(
    ctx: UserContext,
    args: typeof defaultArgs = defaultArgs,
  ) {
  ...
  async function expectError(
    ...
    fragment: RegExp | string,
  ) {
    try {
      await promise;
      throw new Error(
        `Expected error matching "${fragment}", but call succeeded.`,
      );
    } catch (e) {
      if (e instanceof Error && e.message.includes('Expected error matching'))
        throw e;
      const msg: string = e?.toString() ?? '';
      if (typeof fragment === 'string') {
        expect(msg, `error message should contain "${fragment}"`).to.include(
          fragment,
        );
      } else {
        expect(msg, `error message should match ${fragment}`).to.match(
          fragment,
        );
      }
    }
  ...
  describe('initialize_auto_yield', () => {
    it('creates all accounts and seeds initial state correctly', async () => {
      ...
      expect(config.allowedProtocolsMask).to.equal(
        defaultArgs.allowedProtocolsMask,
      );
      expect(config.excludedProtocolsMask).to.equal(
        defaultArgs.excludedProtocolsMask,
      );
    ...
    it('stable_vault authority is the reserve_authority PDA', async () => {
      ...
      expect(vault.owner.toBase58()).to.equal(
        ctx.reserveAuthorityPda.toBase58(),
      );
    });
    ...
    it('rejects double initialization on the same owner', async () => {
      ...
      await expectError(
        initialize(ctx),
        /already in use|already been initialized/i,
      );
    });
    ...
    it('rejects invalid sweep mode (99)', async () => {
      const ctx = await setupUser();
      await expectError(
        initialize(ctx, { ...defaultArgs, sweepMode: 99 }),
        'InvalidSweepMode',
      );
    });
  ...
  describe('update_auto_yield_config', () => {
    it('applies all updated fields to on-chain config', async () => {
      ...
      expect(config.roundUpUnitLamports.toNumber()).to.equal(
        LAMPORTS_PER_SOL / 50,
      );
    });
    ...
    it('rejects update from a non-owner signer', async () => {
      ...
      await expectError(
        program.methods
          .updateAutoYieldConfig(defaultArgs)
  ...
  describe('sweep_stable', () => {
    it('vault receives exact token amount and reserve updates', async () => {
      ...
      expect(reserve.totalSweptStableAtomic.toString()).to.equal(
        amount.toString(),
      );
    });
    ...
    it('rejects stable sweep when disabled', async () => {
      ...
      await initialize(ctx, { ...defaultArgs, enabled: false });
      await expectError(
        sweepStable(ctx, new BN(1_000_000)),
        'AutoYieldDisabled',
      );
    });
  ...
  describe('withdraw_sol', () => {
    ...
    it('SolVaultRentViolation: leaving exactly 1 lamport in vault triggers rent guard', async () => {
      ...
      const _vaultTotal = await solBalance(ctx.solVaultPda);
    ...
    it('rejects withdrawal from non-owner signer', async () => {
      ...
      await expectError(
        program.methods
          .withdrawSol(new BN(LAMPORTS_PER_SOL / 100))
          .accounts({
  ...
  describe('withdraw_stable', () => {
    ...
    it('rejects stable withdrawal from non-owner signer', async () => {
      ...
      await expectError(
        program.methods
          .withdrawStable(new BN(1_000_000))
          .accounts({
            owner: intruder.publicKey,
            config: ctx.configPda,
            reserveState: ctx.reservePda,
    ...
    it('rejects withdraw_stable when ownerStableAccount belongs to a different user', async () => {
      ...
      await expectError(
        program.methods
          .withdrawStable(new BN(10_000_000))
          .accounts({
            owner: ctx.owner.publicKey,
            config: ctx.configPda,
            reserveState: ctx.reservePda,
  ...
  describe('close_empty_reserve', () => {
    it('closes all accounts when reserve is empty and returns rent to owner', async () => {
      ...
      expect(
        await program.account.userAutoYieldConfig.fetchNullable(ctx.configPda),
      ).to.equal(null);
      expect(
        await program.account.userReserveState.fetchNullable(ctx.reservePda),
      ).to.equal(null);
      expect(
        await program.account.solVault.fetchNullable(ctx.solVaultPda),
      ).to.equal(null);
  ...
  describe('multi-user isolation', () => {
    it("two users' PDAs are independent and do not collide", async () => {
      ...
      const aliceReserve = await program.account.userReserveState.fetch(
        alice.reservePda,
      );
      const bobReserve = await program.account.userReserveState.fetch(
        bob.reservePda,
      );

      expect(aliceReserve.solBalanceLamports.toString()).to.equal(
        new BN(LAMPORTS_PER_SOL / 5).toString(),
      );
      expect(bobReserve.solBalanceLamports.toString()).to.equal(
        new BN(LAMPORTS_PER_SOL / 10).toString(),
      );

      // PDAs are distinct
      expect(alice.configPda.toBase58()).to.not.equal(bob.configPda.toBase58());
      expect(alice.reservePda.toBase58()).to.not.equal(
        bob.reservePda.toBase58(),
      );
      expect(alice.solVaultPda.toBase58()).to.not.equal(
        bob.solVaultPda.toBase58(),
      );
    });
    ...
    it('both users can independently close their own reserves', async () => {
      ...
      expect(
        await program.account.userReserveState.fetchNullable(alice.reservePda),
      ).to.equal(null);
      expect(
        await program.account.userReserveState.fetchNullable(bob.reservePda),
      ).to.equal(null);
    });
  });
  ...
  describe('full lifecycle', () => {
    it('init → multi-sweep SOL+stable → partial withdraw → re-sweep → drain → close', async () => {
      ...
      let reserve = await program.account.userReserveState.fetch(
        ctx.reservePda,
      );
      expect(reserve.solBalanceLamports.toString()).to.equal(
        sol1.add(sol2).toString(),
      );
      ...
      expect(reserve.stableBalanceAtomic.toString()).to.equal(
        stab1.sub(stabWithdraw1).toString(),
      );
  ...
  describe('withdraw_vault_sol — happy path', () => {
    ...
    it('can withdraw exactly the sweep amount (vault sits at rent-exempt minimum)', async () => {
      ...
      const vaultInfo = await provider.connection.getAccountInfo(
        ctx.solVaultPda,
      );
      if (!vaultInfo) throw new Error('sol_vault account not found');
      const rentMin =
  ...
  describe('withdraw_vault_sol — validation & rejection', () => {
    ...
    it('rejects u64::MAX withdrawal (arithmetic underflow in rent check)', async () => {
      ...
      const U64_MAX = new BN('18446744073709551615');
      await expectError(withdrawVaultSol(ctx, U64_MAX), 'ArithmeticOverflow');
    });
    ...
    it('rejects withdrawal from a non-owner signer', async () => {
      ...
      await expectError(
        program.methods
          .withdrawVaultSol(new BN(1_000_000))
          .accounts({
            owner: intruder.publicKey,
  ...
  describe('adversarial — cross-path state consistency', () => {
    it('vault lamports from send_sol exceed what UserReserveState tracks', async () => {
      ...
      expect(vaultBalance).to.be.greaterThanOrEqual(
        rentMin + autoSweep.toNumber() + sendSweep,
      );
    });
  ...
  describe('adversarial — account substitution', () => {
    it("send_sol: attacker cannot substitute victim's vault PDA", async () => {
      ...
      await expectError(
        program.methods
          .sendSol(new BN(LAMPORTS_PER_SOL / 10), 100)
          .accounts({
            owner: attacker.owner.publicKey,
  ...
  describe('adversarial — arithmetic boundaries', () => {
    ...
    it('sweep accumulation does not lose precision over many small sends', async () => {
      ...
      const vaultStart = await solBalance(ctx.solVaultPda);
      const N = 10;
      const _perSendSweep = 100_000; // 0.1 SOL * 10 bps / 10_000... wait let me recalculate
