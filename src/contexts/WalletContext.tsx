...
import type { SmartTransactionReview, TokenHolding } from '@helio/types';
...
import {
  resolvePriorityFeeMicroLamportsPerCu,
  reviewNativeSolSend,
...
interface WalletContextType {
  ...
  updateVaultConfig: (args: {
    ...
    roundUpUnitLamports: number;
    deployThresholdAtomic: number;
  }) => Promise<TxResult>;
...
function mapOnChainVault(
  state: OnChainVaultState,
  solPrice: number,
): VaultState {
  const _safe = solPrice > 0 ? solPrice : 145;
...
const DEFAULT_VAULT: VaultState = {
  ...
  lastWithdrawAt: 0,
  rules: {
    roundUpTransfers: false,
    roundUpSwaps: false,
    percentageIncoming: false,
  },
};
...
/** The session Keypair, or throw if the wallet is locked. */
function requireKeypair(): Keypair {
  const kp = loadSessionKeypair();
  if (!kp) {
    throw new Error(
      'Wallet locked — please import your wallet to sign transactions.',
    );
  }
  return kp;
}
...
/** The current wallet address, or throw if none is set. */
function requireWalletAddress(): string {
  const addr = localStorage.getItem(WALLET_ADDRESS_KEY);
  if (!addr) {
    throw new Error('No wallet address — import or create a wallet first.');
  }
  return addr;
}
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const [hasKeypair, _setHasKeypair] = useState(() => !!loadSessionKeypair());
  const [network, setNetwork] = useState<WalletContextType['network']>({
    label: ACTIVE_CLUSTER_LABEL,
    isHealthy: true,
    latencyMs: null,
  });
  ...
  const initializeVault = useCallback(async (): Promise<TxResult> => {
    ...
    const sig = await kitSigner.initializeAutoYield(
      requireSecret(),
      mint.toBase58(),
    );
  ...
  const updateVaultConfig = useCallback(
    async (args: {
      ...
      const addr = requireWalletAddress();
  ...
  const addFundsToVault = useCallback(
    async (amountLamports: number): Promise<TxResult> => {
  ...
  const sweepReserveLamports = useCallback(
    (amountLamports: number, sweepBps: number | null): number => {
  ...
  const submitSend = useCallback(
    async (
      ...
      const priorityFeeMicroLamports =
        await resolvePriorityFeeMicroLamportsPerCu(connection);
      const sig =
        sweepBps !== null
          ? await kitSigner.sendSol(
              secret,
              recipient,
              amountLamports,
              sweepBps,
  ...
  const sendSolWithSweep = useCallback(
    (
      ...
      amountLamports: number,
      sweepBps: number,
    ): Promise<TxResult> => submitSend(recipient, amountLamports, sweepBps),
    [submitSend],
  );

  const sendSolPlain = useCallback(
    (recipient: string, amountLamports: number): Promise<TxResult> =>
      submitSend(recipient, amountLamports, null),
    [submitSend],
  );
  ...
  const validators = useCallback(
    (): Promise<ValidatorInfo[]> => fetchValidators(kitRpc, 25),
    [],
  );
  ...
  const stakeSol = useCallback(
    async (amountLamports: number, votePubkey: string): Promise<TxResult> => {
      const sig = await stakeSigner.stakeAndDelegate(
        requireSecret(),
        amountLamports,
        votePubkey,
      );
  ...
  const deactivateStake = useCallback(
    async (stakeAddress: string): Promise<TxResult> => {
      const sig = await stakeSigner.deactivateStake(
        requireSecret(),
        stakeAddress,
      );
      await fetchDashboard();
  ...
  const withdrawStake = useCallback(
    async (stakeAddress: string, lamports: number): Promise<TxResult> => {
      const sig = await stakeSigner.withdrawStake(
        requireSecret(),
        stakeAddress,
        lamports,
      );
  ...
  const executeSwap = useCallback(
    async (quote: JupiterQuote): Promise<TxResult> => {
      ...
      if (!sim.ok) {
        ...
        throw new Error(
          `Swap simulation blocked this transaction: ${sim.reason}`,
        );
      }
      const sig = await signSendSwap(
        swapConnection,
        vtx,
        kp,
  ...
  const updateVault = useCallback(
    (updates: Partial<VaultState>) => setVault((v) => ({ ...v, ...updates })),
    [],
  );

  const updateVaultRule = useCallback(
    (key: keyof VaultState['rules'], value: boolean) =>
  ...
  const setWalletAddress = useCallback(
    (address: string, label = 'Main Wallet') => {
