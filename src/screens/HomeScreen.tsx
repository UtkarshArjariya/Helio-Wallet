...
function _fmtDate(unix: number): string {
  if (!unix) return '';
...
export function HomeScreen() {
  const { totalBalanceUsd, tokens, vault, loading, error, refresh, address } =
    useWallet();
  ...
  const progress =
    vault.threshold > 0
      ...
      : 0;
  const _solToken = tokens.find((t) => t.id === 'sol');
  ...
  return (
    <div ref={scrollRef} className="space-y-3 p-4">
      ...
      {error && (
        <div
          className="flex items-center gap-2 rounded-2xl border px-3 py-2.5 text-xs text-danger"
          style={{
        ...
        >
          ...
          <button
            type="button"
            onClick={refresh}
            className="text-danger underline shrink-0"
          >
            Retry
          </button>
        </div>
      ...
      {!dismissed && (
        ...
        >
          <span
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-accent-primary"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-text-muted text-xs">
              Get notified for deposits, thresholds, and rewards.
            </div>
          </div>
          <button
            type="button"
      ...
      <FadeUp className="relative overflow-hidden rounded-3xl helio-card helio-noise p-5">
        ...
        <div className="relative z-20 flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              aria-label="Refresh"
              className={cn(
                'flex h-7 w-7 items-center justify-center rounded-full text-text-muted hover:text-text-primary transition-colors',
        ...
        <div className="relative z-10 mb-5">
          <p className="font-eyebrow text-text-muted text-[10px] mb-1.5">
            Total Balance
          </p>
        ...
        <div className="relative z-10 grid grid-cols-4 gap-2">
          {QUICK_ACTIONS.map(
            ({ label, index, icon: Icon, path, primary, accent }) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      color: primary ? 'rgba(0,0,0,0.55)' : 'var(--text-muted)',
                    }}
                  >
                    {index}
                  </span>
        ...
        >
          <span className="text-accent-primary shrink-0">
            <VaultGlyph size={14} strokeWidth={1.8} />
          </span>
        ...
        <div className="rounded-2xl helio-card p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              {
                label: 'Balance',
                value: `${vault.balance.toFixed(3)} SOL`,
                accent: true,
              },
              {
                label: 'Est. APY',
                value: vault.isActive ? '7.1%' : 'Inactive',
                accent: vault.isActive,
              },
              ...
              >
                ...
                <div
                  className={cn(
                    'mt-1 text-lg font-semibold font-heading',
      ...
      {tab === 'staking' && (
        <div className="rounded-2xl helio-card p-5">
          ...
          <p className="text-text-muted text-xs mt-2 leading-relaxed max-w-[36ch]">
            Native validator delegation and liquid staking land soon. In the
            meantime your Vault still accumulates round-ups on every send.
          </p>
          <button
            type="button"
            onClick={() => navigate('/vault')}
