...
export function ActivityScreen() {
  ...
  const {
    ...
    error,
    refresh,
  } = useRecentTransactions(address, 30);
  ...
  const filtered = useMemo(() => {
    return allActivity
      .filter((a) => {
        ...
        if (filter === 'stake')
          return a.kind === 'stake' || a.kind === 'unstake';
          ...
          {FILTERS.map((f) => (
            <button
              ...
              style={{
                ...
                borderColor:
                  filter === f.id ? 'transparent' : 'var(--border-subtle)',
              }}
            >
              {f.label}
            </button>
        ...
        {grouped.length === 0 ? (
          loading && allActivity.length === 0 ? (
            <div className="rounded-3xl helio-card p-10 flex flex-col items-center gap-2 text-center">
              ...
              <div className="text-text-muted text-[11px]">
                Pulling recent signatures from the cluster.
              </div>
            </div>
            ...
            <EmptyState
              ...
              body={
                <>
                  Send or receive SOL, or set up the Vault to start round-up
                  savings. Every transaction will surface here, indexed by day.
                </>
              }
              primary={{
                label: 'Send SOL',
          ...
          grouped.map(([day, items]) => (
            <div key={day} className="rounded-3xl helio-card p-2">
              <div className="px-3 py-2 font-eyebrow text-text-muted text-[10px]">
                {day}
              </div>
