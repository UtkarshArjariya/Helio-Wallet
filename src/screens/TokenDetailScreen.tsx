...
function rangeToFetch(range: Range): {
  interval: JupiterChartInterval;
  count: number;
} {
  switch (range) {
    case '1H':
      return { interval: '1_MINUTE', count: 60 };
    case '1D':
      return { interval: '5_MINUTE', count: 288 };
    case '1W':
      return { interval: '1_HOUR', count: 168 };
    case '1M':
      return { interval: '1_DAY', count: 30 };
    case 'YTD': {
    ...
    case 'ALL':
      return { interval: '1_DAY', count: 365 };
  }
}
...
export function TokenDetailScreen() {
  ...
  const mintToFetch =
    held?.mintAddress ?? (id === 'sol' ? WRAPPED_SOL_MINT : id);
  ...
  useEffect(() => {
    ...
    jupiterChartsClient
      ...
      .catch((e) => {
        if (!cancelled) setChartErr(e?.message ?? 'Failed to load chart');
      })
      .finally(() => {
  ...
  const chartChange =
    closes.length >= 2 && firstClose !== undefined && lastClose !== undefined
    ...
    <div className="flex flex-col pb-16">
      ...
      <div className="relative mx-auto w-full max-w-[64rem] px-4 md:px-8 lg:px-12 pt-3">
        ...
        <div className="relative" style={{ isolation: 'isolate' }}>
            ...
            >
              ...
              <div className="h-56 md:h-72 relative">
                {chartLoading && closes.length === 0 ? (
                  ...
                  <Centered column>
                    <div className="text-text-primary/30 font-figure text-3xl font-extrabold tabular-nums">
                      ⋯
                    </div>
              ...
              >
                <div>
                  ...
                  <div className="mt-1 font-figure text-text-primary text-2xl md:text-[28px] font-extrabold tabular-nums leading-none">
                    ...
                    <span className="ml-2 text-text-muted text-base font-bold">
                      {symbol}
                    </span>
                  </div>
                </div>
          ...
          <motion.section
            {...stagger(4)}
            className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6"
          >
          ...
          <motion.section {...stagger(5)} className="pt-8">
            <p className="text-text-muted text-[11px] leading-relaxed max-w-2xl mx-auto md:mx-0 text-center md:text-left">
              Past performance is not an indicator of future performance. Token
              prices are sourced from Jupiter aggregator routing across Solana
              DEXs and may differ from individual venue prices.
            </p>
          </motion.section>
        </div>
      </div>
  ...
  return (
    <div className="py-2" aria-hidden>
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(to right, transparent, var(--text-muted) 30%, var(--text-muted) 70%, transparent)',
          opacity: 0.5,
        }}
      />
      <div
...
function DataRow({
  ...
  return (
    <div className="flex items-baseline gap-3 py-2.5">
      <span className="text-text-muted text-xs shrink-0 uppercase tracking-wider font-mono">
        {label}
      </span>
      <span
