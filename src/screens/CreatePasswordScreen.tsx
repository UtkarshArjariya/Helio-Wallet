...
export function CreatePasswordScreen() {
  ...
  const isValid =
    password.length >= 8 && password === confirmPassword && agreed && !pending;
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (mode === 'import') {
        ...
        if (!p)
          throw new Error('No recovery phrase found. Please re-enter it.');
        phrase = p;
      ...
      const vault = await encryptVault(
        { secretKey: keypair.secretKey, phrase },
        password,
      );
        ...
        <CardContent className="p-6 flex-1 flex flex-col space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3 pt-1">
              ...
              >
                I agree to the{' '}
                <span className="text-accent-primary hover:underline cursor-pointer">
                  Terms of Service
                </span>{' '}
                and{' '}
                <span className="text-accent-primary hover:underline cursor-pointer">
