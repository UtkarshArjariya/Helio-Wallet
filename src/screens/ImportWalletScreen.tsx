...
export function ImportWalletScreen() {
  ...
  const handleContinue = () => {
    ...
    if (!isValidPhrase(phrase)) {
      setError(
        'That phrase is not a valid BIP-39 mnemonic. Check the order and spelling.',
      );
      return;
    }
        ...
        <CardContent className="p-6 md:p-8 flex-1 flex flex-col space-y-6 overflow-y-auto">
          <div className="space-y-2 text-center md:text-left">
            ...
            <p className="text-sm text-text-muted">
              Enter your 12 or 24-word recovery phrase. You can paste the whole
              phrase into the first box.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
            {words.map((word, index) => (
              // biome-ignore lint/suspicious/noArrayIndexKey: each slot is a fixed positional input (word N), the list is never reordered, and words may be empty/duplicate so no content-based key is stable
          ...
          <div className="mt-auto pt-6">
            <div className="p-4 bg-warning/10 rounded-xl border border-warning/20 mb-6">
              <p className="text-xs text-warning leading-relaxed">
                Never share your recovery phrase with anyone. Anyone with this
                phrase can access your assets.
              </p>
            </div>
            <Button
              className="w-full"
