...
import {
  AlertTriangle, Check, CheckCircle, ChevronDown, ExternalLink, Loader2, Layers,
} from 'lucide-react'
...
const STATUS_STYLE: Record<StakeAccountInfo['status'], { label: string; color: string }> = {
...
export function StakingScreen() {
  const { tokens, hasKeypair, stakeAccounts, validators, stakeSol, deactivateStake, withdrawStake } = useWallet()
  ...
  const refreshPositions = useCallback(async () => {
    ...
    try { setPositions(await stakeAccounts()) }
    catch (e: any) {
      setError(e?.message ? `Could not load stake accounts: ${e.message}` : 'Could not load stake accounts.')
    }
  ...
  const canStake = hasKeypair && numeric > 0 && numeric <= balance && selected !== null && busy === null

  const handleStake = async () => {
    if (!canStake || !selected) return
    setBusy('stake'); setError(null); setResult(null)
    try {
  ...
  const runOp = async (id: string, op: () => Promise<{ signature: string; explorerUrl: string }>) => {
      ...
      <div className="relative p-4 space-y-4" style={{ isolation: 'isolate' }}>
        ...
        <div className="rounded-3xl helio-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <span className="font-eyebrow text-text-muted text-[10px]">Amount to stake</span>
            <button type="button" onClick={() => setAmount(String(Math.max(0, balance - STAKE_RESERVE_SOL)))}
          ...
          <div className="text-xs text-text-muted font-mono text-right">Balance: {balance.toFixed(4)} SOL</div>
          ...
          <button type="button" onClick={() => setPickerOpen(v => !v)}
            ...
            <div className="min-w-0">
              <div className="font-eyebrow text-text-muted text-[10px]">Validator</div>
            ...
            {selected && (
              <div className="text-right shrink-0 ml-2">
                <div className="text-text-secondary text-xs">{selected.commission}% fee</div>
                <ChevronDown className={cn('h-4 w-4 text-text-muted inline transition-transform', pickerOpen && 'rotate-180')} />
              </div>
            )}
          </button>

          {pickerOpen && (
            <div className="max-h-56 overflow-y-auto helio-scrollbar rounded-2xl border divide-y"
              style={{ borderColor: 'var(--border-subtle)', background: 'var(--surface-2)' }}>
              {vals.length === 0 && (
                <div className="px-4 py-3 text-text-muted text-xs">No validators available on this cluster.</div>
              )}
              {vals.map(v => (
                <button key={v.votePubkey} type="button"
                  onClick={() => { setSelected(v); setPickerOpen(false) }}
                  className="flex w-full items-center justify-between px-4 py-2.5 text-left hover:bg-surface-3 transition-colors"
                  style={{ borderColor: 'var(--border-subtle)' }}>
                  <span className="text-text-primary text-sm font-mono">{short(v.votePubkey)}</span>
                  <span className="text-text-muted text-xs">
                    {v.commission}% · {v.activatedStakeSol.toLocaleString('en-US', { maximumFractionDigits: 0 })} SOL
                  </span>
                </button>
              ))}
            </div>
          ...
          <button type="button" onClick={handleStake} disabled={!canStake}
            className={cn(
              'flex w-full items-center justify-center gap-2 rounded-full py-3.5 text-sm font-semibold transition-colors',
              canStake ? 'bg-accent-primary text-accent-primary-foreground hover:bg-accent-primary-hover' : 'text-text-muted cursor-not-allowed',
            )}
            style={!canStake ? { background: 'var(--surface-3)' } : {}}>
            {busy === 'stake' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Layers className="h-4 w-4" />}
            {busy === 'stake' ? 'Delegating…'
              : numeric > balance ? 'Insufficient balance'
        ...
        {result && (
          <a href={result.url} target="_blank" rel="noopener noreferrer"
            ...
            <span className="flex-1">Transaction confirmed · {result.sig.slice(0, 16)}…</span>
        ...
        {/* Positions */}
        <div>
          <div className="font-eyebrow text-text-muted text-[10px] px-1 mb-2">Your stake accounts</div>
          {loadingPositions ? (
            ...
            <div className="rounded-2xl helio-card p-4 text-text-muted text-xs text-center">
              No stake accounts yet. Delegate above to start earning epoch rewards.
            </div>
          ) : (
              ...
              {positions.map(p => {
                ...
                return (
                  <div key={p.address} className="rounded-2xl helio-card p-4">
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        <div className="text-text-primary text-sm font-mono">{short(p.address)}</div>
                      ...
                      <div className="text-right shrink-0 ml-2">
                        ...
                        <span className="font-eyebrow text-[10px]" style={{ color: st.color }}>{st.label}</span>
                      </div>
                    </div>
                    <div className="mt-3 flex gap-2">
                      {(p.status === 'active' || p.status === 'activating') && (
                        <button type="button" disabled={inFlight}
                          onClick={() => runOp(opId, () => deactivateStake(p.address))}
                      ...
                      {p.status === 'inactive' && (
                        <button type="button" disabled={inFlight}
                          onClick={() => runOp(opId, () => withdrawStake(p.address, p.lamports))}
                          ...
                          {inFlight ? 'Working…' : <><Check className="h-3 w-3" /> Withdraw all</>}
                        </button>
                      )}
                    </div>
                  </div>
        ...
        <p className="text-text-muted text-[11px] leading-relaxed px-1">
          Native staking earns epoch rewards (~2–3 days/epoch). Deactivation takes one full epoch
          before funds become withdrawable. Stake accounts keep a small rent reserve.
        </p>
      </div>
    </div>
  )
