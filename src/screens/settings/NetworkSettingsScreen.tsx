...
import { NETWORKS, type NetworkCode, useNetwork } from '../../lib/preferences';
...
function clusterUrls(): Record<NetworkCode, string> {
  ...
  return {
    mainnet:
      cfg.rpcEndpointPool['mainnet-beta'][0]?.url ??
      'https://api.mainnet-beta.solana.com',
    testnet: 'https://api.testnet.solana.com',
    devnet:
      cfg.rpcEndpointPool.devnet[0]?.url ?? 'https://api.devnet.solana.com',
  };
}
  ...
  useEffect(() => {
    ...
    for (const [code, url] of Object.entries(urls) as [NetworkCode, string][]) {
      ...
      const timeoutId = setTimeout(() => {
        if (cancelled) return;
        setLatencies((prev) =>
          prev[code] === 'probing' ? { ...prev, [code]: 'unreachable' } : prev,
        );
      }, PROBE_TIMEOUT_MS);
      ...
      conn
        ...
        .then(() => {
          ...
          setLatencies((prev) => ({
            ...prev,
            [code]: Math.round(performance.now() - start),
          }));
        })
        .catch(() => {
          if (cancelled) return;
...
function latencyLabel(state: LatencyState): {
  text: string;
    ...
    <div className="flex flex-col">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="rounded-3xl helio-card p-4 flex items-center gap-3">
          <span
            className={`h-2 w-2 rounded-full ${network.isHealthy ? 'bg-accent-primary helio-pulse-ring' : 'bg-danger'}`}
          />
          ...
          {NETWORKS.map((n) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={cn('font-mono text-[11px] shrink-0', toneClass)}
                >
                  {lbl.text}
                </div>
                {active && (
        ...
        {current !== 'mainnet' && (
          ...
          >
            ...
            <span className="leading-relaxed">
              Testnet and devnet balances are not real. Tokens received here
              have no monetary value and may be reset.
            </span>
          </div>
        )}
      </div>
