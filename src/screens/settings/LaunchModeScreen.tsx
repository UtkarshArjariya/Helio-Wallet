...
import { type LaunchMode, useLaunchMode } from '../../lib/launch-mode';
...
const OPTIONS: ModeOption[] = [
  {
    ...
    label: 'Side panel',
    description:
      'Persistent column on the right of the browser. Stays open while you browse — best for active trading.',
    icon: PanelRight,
  },
  {
    value: 'popup',
    label: 'Popup',
    description:
      'Classic toolbar popup. Compact, opens above the page, dismisses when you click away.',
  ...
  },
  {
    ...
    label: 'Full tab',
    description:
      'Opens in a regular browser tab — the most room for the vault dashboard and activity views.',
    icon: Maximize2,
  },
];
...
export function LaunchModeScreen() {
  ...
  return (
    <div className="flex flex-col">
      <ScreenHeader
        title="Launch mode"
        subtitle="How the wallet opens from the toolbar"
      />
      ...
      <div className="p-4 space-y-4">
        <SettingsSection label="Open as">
          {OPTIONS.map((opt) => {
            ...
            return (
              ...
              >
                <span
                  className="flex h-9 w-9 items-center justify-center rounded-xl text-text-secondary shrink-0 mt-0.5"
        ...
        >
          ...
          <span className="text-text-muted leading-relaxed">
            Side panel requires Chrome 114 or newer. On unsupported browsers the
            wallet falls back to popup mode automatically.
          </span>
        </div>

        <p className="text-text-muted text-[11px] px-1 leading-relaxed">
          Changes take effect the next time you click the Helio icon in your
          browser toolbar.
        </p>
      </div>
    </div>
