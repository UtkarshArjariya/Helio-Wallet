  ...
  return (
    <RevealFlow
      ...
      render={(key, revealed) => (
        ...
        >
          <div className="font-eyebrow text-text-muted text-[10px] mb-2">
            Base58
          </div>
          <div
            className="font-mono text-xs text-text-primary break-all leading-relaxed select-text"
            style={{ filter: revealed ? 'none' : 'blur(6px)' }}
          >
            {
              revealed
                ? key
                : key /* placeholder still keeps layout — blur masks it */
            }
          </div>
        </div>
      )}
