...
function toAtomic(display: string, decimals: number): string | null {
  if (!/^\d*\.?\d*$/.test(display) || display === '' || display === '.')
    return null;
...
export function SwapScreen() {
  ...
  const refreshBalance = useCallback(async () => {
    ...
    try {
      ...
      if (sellToken.mint === SOL.mint) {
        ...
        const res = await swapConnection.getParsedTokenAccountsByOwner(owner, {
          mint: new PublicKey(sellToken.mint),
        });
        const ui =
  ...
  useEffect(() => {
    ...
    if (!atomic || atomic === '0' || sellToken.mint === buyToken.mint) {
      setQuote(null);
      setQuoting(false);
      return;
    }
  ...
  const minReceived = quote
    ? fromAtomic(quote.otherAmountThreshold, buyToken.decimals)
    : 0;
    ...
    <div className="flex flex-col">
      ...
      <div className="p-4 space-y-2">
        ...
        <div
          ...
          style={{
            background: 'rgba(26,31,184,0.08)',
            borderColor: 'rgba(26,31,184,0.25)',
        ...
        >
          ...
          <span>
            Swaps run on{' '}
            <span className="font-semibold text-text-primary">Mainnet</span>{' '}
            (Jupiter liquidity). Balances below are your mainnet balances.
          </span>
        </div>
        ...
        <SwapCard
          ...
          amount={
            quote
              ? outDisplay.toLocaleString('en-US', { maximumFractionDigits: 6 })
              : ''
          }
          readOnly
          loading={quoting}
      ...
      {quote && (
        <div className="px-4 pt-2">
          <div className="rounded-2xl helio-card p-4 space-y-2 text-xs">
            <Row
              label="Rate"
              value={`1 ${sellToken.symbol} ≈ ${(outDisplay / (numericSell || 1)).toLocaleString('en-US', { maximumFractionDigits: 6 })} ${buyToken.symbol}`}
            />
            <Row
              label="Minimum received"
              value={`${minReceived.toLocaleString('en-US', { maximumFractionDigits: 6 })} ${buyToken.symbol}`}
            />
            <Row
              label="Price impact"
              value={`${impact.toFixed(2)}%`}
              accent={
                impact >= 1 ? (impact >= 5 ? 'danger' : 'warn') : undefined
              }
            />
        ...
        {result && (
          ...
          >
            ...
            <span className="flex-1">
              Swap confirmed · {result.sig.slice(0, 16)}…
            </span>
  ...
  return (
    <div className="rounded-3xl helio-card p-5">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <TokenIcon
            token={{ symbol: token.symbol, iconUrl: token.icon }}
            size={24}
          />
          {token.symbol}
...
function TokenPicker({
  ...
  useEffect(() => {
    const q = query.trim();
    if (q.length < 2) {
      setRows([SOL, USDC].filter((t) => t.mint !== excludeMint));
      return;
    }
    let cancelled = false;
    setSearching(true);
      ...
      >
        ...
        <div className="mt-3 max-h-[50vh] overflow-y-auto helio-scrollbar space-y-0.5">
          ...
          {rows.map((t) => (
            ...
            >
              <TokenIcon
                token={{ symbol: t.symbol, iconUrl: t.icon }}
                size={32}
              />
