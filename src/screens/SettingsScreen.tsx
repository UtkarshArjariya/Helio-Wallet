...
export function SettingsScreen() {
  ...
  const networkLabel =
    NETWORKS.find((n) => n.code === network)?.label ?? 'Mainnet Beta';
  const themeLabel =
    THEMES.find((t) => t.code === theme)?.label ?? 'Solar Midnight';
  const autolockLbl =
    AUTOLOCK_OPTIONS.find((o) => o.value === autolock)?.label ?? '5 minutes';
  const launchLbl =
    launchMode === 'sidebar'
      ? 'Side panel'
      : launchMode === 'popup'
        ? 'Popup'
        : 'Full tab';
  const activeVaultAlerts = [
    notifs.vaultThresholdReached,
    notifs.vaultRewards,
      ...
      items: [
        {
          ...
          sub: notifs.push ? 'Enabled' : 'Disabled',
        ...
        },
        {
          ...
          icon: Sparkles,
          label: 'Vault alerts',
          sub: `${activeVaultAlerts} of 2 active`,
          path: '/settings/vault-alerts',
        },
      ],
    },
    {
      ...
      items: [
        {
          ...
          icon: ShieldCheck,
          label: 'Manage apps',
          sub: 'No dApps connected',
          path: '/settings/manage-apps',
        },
        {
          id: 'approvals',
          icon: ShieldCheck,
          label: 'Spending approvals',
          sub: 'No standing approvals',
          path: '/settings/spending-approvals',
        },
        {
          id: 'autolock',
          icon: Timer,
          label: 'Auto-lock',
          sub: `After ${autolockLbl}`,
