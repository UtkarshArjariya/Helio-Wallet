...
function write<T>(key: string, value: T): void {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('helio:pref', { detail: { key, value } }),
    );
  }
}
...
export function usePref<T>(key: string, fallback: T): [T, (v: T) => void] {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: re-subscribes only when `key` changes; `fallback` is intentionally not a dep to avoid re-subscribing on an inline fallback's changing identity.
  useEffect(() => {
...
export const useLanguage = () =>
  usePref<LanguageCode>(KEY.language, DEFAULTS.language);
export const useCurrency = () =>
  usePref<CurrencyCode>(KEY.currency, DEFAULTS.currency);
export const useNetwork = () =>
  usePref<NetworkCode>(KEY.network, DEFAULTS.network);
export const useTheme = () => usePref<ThemeCode>(KEY.theme, DEFAULTS.theme);
export const useAutoLock = () =>
  usePref<AutoLockMs>(KEY.autoLock, DEFAULTS.autoLock);
export const useNotifications = () =>
  usePref<NotificationPrefs>(KEY.notifications, DEFAULTS.notifications);
...
export const NETWORKS: {
  ...
  { code: 'testnet', label: 'Testnet', subtitle: 'Pre-release · resettable' },
  {
    code: 'devnet',
    label: 'Devnet',
    subtitle: 'Developer airdrops · resettable',
  },
];
...
export const THEMES: {
...
}[] = [
  {
    ...
    description: 'Default · lime on jet',
    swatch: ['#000000', '#C6F000', '#1A1FB8'],
  },
  {
    code: 'pure-noir',
    label: 'Pure Noir',
    description: 'Monochrome · pure restraint',
    swatch: ['#000000', '#FFFFFF', '#7A7A7A'],
  },
  {
    code: 'cosmic-blue',
    label: 'Cosmic Blue',
    description: 'Royal blue dominant',
    swatch: ['#000000', '#1A1FB8', '#C6F000'],
  },
];
