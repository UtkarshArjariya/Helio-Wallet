...
import { createTokenBucket, isAllowedRpcUrl, validateRpcUrl } from './rpc-guard'

describe('validateRpcUrl — scheme allowlist', () => {
  it('accepts https endpoints', () => {
    expect(validateRpcUrl('https://api.mainnet-beta.solana.com')).toContain('https://')
  ...
  it('rejects cleartext http to remote hosts', () => {
    ...
    expect(() => validateRpcUrl('http://api.mainnet-beta.solana.com')).toThrow()
  })
