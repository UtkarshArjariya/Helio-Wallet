 ...
 */
function createLocalStorageStubAdapter(): TokenStorageAdapter & {
  reset: () => void;
} {
...
const adapters: { name: string; make: () => TokenStorageAdapter }[] = [
  {
    name: 'memory adapter (chrome.storage.local contract)',
    make: () => createMemoryAdapter(),
  },
  {
    name: 'localStorage stub adapter',
    make: () => createLocalStorageStubAdapter(),
  },
];
