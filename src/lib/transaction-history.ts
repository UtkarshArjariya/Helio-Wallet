...
import type {
  ActivityItem,
  ActivityKind,
} from '../components/wallet/ui/ActivityRow';
...
const HELIO_DISCRIMINATORS: { name: HelioIx; bytes: number[] }[] = [
  ...
  {
    name: 'initialize_auto_yield',
    bytes: [251, 132, 53, 164, 110, 171, 181, 23],
  },
  ...
  {
    name: 'update_auto_yield_config',
    bytes: [36, 48, 204, 198, 62, 156, 39, 92],
  },
...
> = {
  initialize_auto_yield: {
    kind: 'vault-deploy',
    title: 'Vault created',
    subtitle: 'Auto-yield initialised',
  },
  pause_auto_yield: {
    kind: 'vault-deploy',
    title: 'Vault paused',
    subtitle: 'Round-ups halted',
  },
  resume_auto_yield: {
    kind: 'vault-deploy',
    title: 'Vault resumed',
    subtitle: 'Round-ups re-enabled',
  },
  update_auto_yield_config: {
    kind: 'vault-deploy',
    title: 'Vault rules updated',
    subtitle: 'Config changed',
  },
  send_sol: {
    kind: 'send',
    title: 'Sent SOL + round-up',
    subtitle: 'Helio send w/ vault sweep',
  },
  sweep_sol: {
    kind: 'vault-roundup',
    title: 'Vault deposit',
    subtitle: 'Swept into SOL reserve',
  },
  sweep_stable: {
    kind: 'vault-roundup',
    title: 'Stable vault deposit',
    subtitle: 'Swept into stable reserve',
  },
  withdraw_sol: {
    kind: 'vault-withdraw',
    title: 'Vault withdrawal',
    subtitle: 'SOL back to wallet',
  },
  withdraw_vault_sol: {
    kind: 'vault-withdraw',
    title: 'Vault withdrawal',
    subtitle: 'Reserve → wallet',
  },
  withdraw_stable: {
    kind: 'vault-withdraw',
    title: 'Stable withdrawal',
    subtitle: 'Stable reserve → wallet',
  },
  close_empty_reserve: {
    kind: 'vault-deploy',
    title: 'Reserve closed',
    subtitle: 'Empty reserve account closed',
  },
};
  ...
  for (let i = 0; i < keys.length; i++) {
    const k =
      typeof keys[i] === 'string'
        ? (keys[i] as unknown as string)
        : keys[i].pubkey.toString();
    if (k === owner) return i;
  }
  return -1;
}
...
function netSolDelta(tx: ParsedTransactionWithMeta, owner: string): number {
  ...
  const fee =
    key && typeof key === 'object' && (key as ParsedMessageAccount).signer
      ? (tx.meta.fee ?? 0)
      : 0;
...
function systemTransferCounterparty(
  tx: ParsedTransactionWithMeta,
  owner: string,
): string | null {
  for (const ix of tx.transaction.message.instructions) {
...
function classify(
  ...
  const isToken =
    programs.has(TOKEN_PROGRAM) || programs.has(TOKEN_2022_PROGRAM);
    ...
    if (ixName) {
      ...
      const amount =
        Math.abs(solDelta) > 0.0000001
          ...
          : '';
      const positive =
        meta.kind === 'vault-roundup'
          ? false // outflow from wallet → vault
          : meta.kind === 'vault-withdraw'
            ? true // inflow from vault → wallet
            : meta.kind === 'send'
              ? false
              : solDelta > 0
                ? true
  ...
  if (
    onlySystem ||
    (programs.has(SYSTEM_PROGRAM) && Math.abs(solDelta) > 0.000001)
  ) {
    ...
    return {
      ...
      subtitle: counterparty
        ? positive
          ? `From ${shortAddr(counterparty)}`
          : `To ${shortAddr(counterparty)}`
        : 'SOL transfer',
      amount: `${positive ? '+' : ''}${solDelta.toFixed(4)} SOL`,
      positive,
    };
...
export async function fetchRecentTransactions(
  ...
  if (!BASE58_RE.test(address)) {
    ...
    throw new Error(
      `Invalid wallet address shape (got "${address.slice(0, 16)}…")`,
    );
  }
