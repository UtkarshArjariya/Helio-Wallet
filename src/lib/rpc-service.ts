...
function resolveCluster(): 'mainnet-beta' | 'devnet' {
  try {
    const raw =
      ...
      localStorage.getItem('helio:pref:network');
...
function urlFor(cluster: 'mainnet-beta' | 'devnet'): string {
  if (cluster === 'devnet') {
    return (
      config.rpcEndpointPool.devnet[0]?.url ??
      import.meta.env.VITE_HELIO_DEVNET_RPC_PRIMARY_URL ??
      'https://api.devnet.solana.com'
    );
  }
  return (
    config.rpcEndpointPool['mainnet-beta'][0]?.url ??
