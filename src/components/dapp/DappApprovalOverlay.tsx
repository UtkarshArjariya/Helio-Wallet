import type { DappTrustLevel, PendingDappRequest } from '@helio/types';
import {
  AlertTriangle,
  Globe,
  Loader2,
  ...
  return new Promise((resolve) => {
    ...
    try {
      chrome.runtime.sendMessage(
        { type, payload },
        (res: BackgroundResponse | null | undefined) => {
  ...
  switch (req.kind) {
    case 'connect':
      return {
        ...
        body: 'It will be able to see your wallet address and request transactions and signatures.',
      };
    case 'sign-transaction':
      return {
        title: 'wants you to approve a transaction',
        body: 'Review the request carefully — approving will sign a transaction with your wallet.',
      };
    case 'sign-message':
...
export function DappApprovalOverlay() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (cancelled) return;
      setPending(
        (res?.ok ? (res.data as PendingDappRequest | null) : null) ?? null,
      );
  ...
  const safeIcon =
    pending.dapp.iconUrl && /^https:\/\//i.test(pending.dapp.iconUrl)
      ? pending.dapp.iconUrl
      : null;
    ...
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-3xl helio-card p-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <span
            className="flex h-11 w-11 items-center justify-center rounded-2xl overflow-hidden shrink-0"
          ...
          <div className="min-w-0">
            <div className="text-text-primary font-heading font-semibold text-base truncate">
              {pending.dapp.name}
            </div>
          ...
          <span
            className="ml-auto inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-eyebrow shrink-0"
            style={{
              background: `color-mix(in srgb, ${trust.color} 14%, transparent)`,
              color: trust.color,
            }}
          >
        ...
        {warnings.length > 0 && (
          <div className="space-y-2">
            {warnings.map((w) => (
              <div
                key={`${w.code}-${w.severity}-${w.title}-${w.message}`}
        ...
        {error && (
          <div
            className="flex items-start gap-2 rounded-xl border p-3 text-xs text-danger"
            style={{
