...
export function BottomNav() {
  ...
  return (
    <nav
      className="sticky bottom-0 z-30 grid grid-cols-4 gap-1 border-t backdrop-blur-md px-2 pt-1.5 pb-3"
    ...
    >
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            {active && <span className="absolute inset-x-4 top-0 h-0.5 rounded-full bg-accent-primary" />}
            <span
              className={cn(
              ...
              style={
                item.highlight && !active ? { background: 'rgba(198,240,0,0.1)' }
                : active && !item.highlight ? { background: 'var(--surface-3)' }
                : {}
              }
            >
