...
function NotificationPanel({ onClose }: { onClose: () => void }) {
  ...
  const grouped = useMemo(() => {
    const map = new Map<
      string,
      { label: string; order: number; items: Notification[] }
    >();
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span className="font-heading text-text-primary font-semibold text-sm">
            Notifications
          </span>
          {unread > 0 && (
      ...
      <div className="max-h-[400px] overflow-y-auto helio-scrollbar">
        {notifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-6 py-10 text-center gap-2">
            ...
            <div className="font-figure text-text-primary/30 text-3xl font-extrabold tabular-nums select-none leading-none">
              —
            </div>
          ...
          grouped.map((group, gi) => (
            <div key={group.label}>
              ...
              >
                ...
                />
                <span className="font-eyebrow text-text-muted text-[9px]">
                  {group.label}
                </span>
                <span
                  className="h-px flex-1"
...
function PopupTopHeader() {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative flex items-center justify-between gap-2 px-4 py-3 border-b"
        style={{ borderColor: 'var(--border-subtle)' }}
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className={cn(
              'relative flex h-8 w-8 items-center justify-center rounded-full text-text-secondary hover:text-text-primary transition-colors',
...
function DesktopTopHeader() {
  ...
  return (
    <header style={{ background: 'rgba(0,0,0,0.6)' }}>
      ...
      <div
        className="relative flex items-center justify-between gap-4 px-6 py-4 border-b"
        style={{ borderColor: 'var(--border-subtle)' }}
      >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={handleBellClick}
            className={cn(
              'relative flex h-9 w-9 items-center justify-center rounded-full border text-text-secondary hover:bg-surface-3 hover:text-text-primary transition-colors',
          ...
          >
            ...
            {hasUnread && (
              <span
                className="absolute right-2 top-2 h-2 w-2 rounded-full bg-accent-primary"
                style={{ outline: '2px solid var(--bg)' }}
              />
            )}
          </button>
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  if (chromeless) {
    return (
      <div
        className="helio-bg-atmosphere flex h-screen w-full flex-col overflow-hidden"
    ...
    <>
      ...
      <div className="helio-bg-atmosphere hidden md:flex h-screen w-full overflow-hidden">
        ...
        <div className="relative z-10 flex min-w-0 flex-1 flex-col">
          <DesktopTopHeader />
          <main className="flex-1 overflow-y-auto helio-scrollbar">
            {children}
          </main>
        </div>
      </div>
      ...
      {/* Popup / mobile */}
      <div
        className="helio-bg-atmosphere md:hidden flex h-screen w-full flex-col"
        style={{ minWidth: 360, minHeight: 600 }}
      >
        <div className="relative z-10 flex flex-1 min-h-0 flex-col">
          <PopupTopHeader />
          <main className="flex-1 overflow-y-auto helio-scrollbar overscroll-contain">
            {children}
          </main>
          <BottomNav />
        </div>
