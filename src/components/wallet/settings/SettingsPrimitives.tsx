...
export function SettingsSection({
  ...
  return (
    <section className={cn('space-y-2', className)}>
      {label && (
        <div className="px-1 font-eyebrow text-text-muted text-[10px]">
          {label}
        </div>
      )}
      <div className="rounded-3xl helio-card overflow-hidden">{children}</div>
    </section>
  );
}
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3">
      ...
      <button
        ...
        className={cn(
          'relative h-5 w-9 shrink-0 rounded-full mt-0.5 transition-colors',
