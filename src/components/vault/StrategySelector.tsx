...
import { useWallet, type VaultState } from '../../contexts/WalletContext';
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choose where funds go after the threshold is met.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {strategies.map((strategy) => (
          <button
            ...
            key={strategy.id}
            onClick={() =>
              updateVault({ strategy: strategy.id as VaultState['strategy'] })
            }
            className={cn(
              'w-full text-left flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all',
