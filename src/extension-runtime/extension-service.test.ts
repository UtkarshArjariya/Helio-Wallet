...
describe('extension-service auto-yield flow', () => {
  it('updates auto-yield settings and exposes the normalized state', async () => {
    ...
    if (!currentSettings) {
      throw new Error('expected auto-yield settings to be defined');
    }
  ...
  it('sweeps SOL into the reserve and records a manual deploy', async () => {
    ...
    const dashboard = runtimeSnapshot.dashboard;
    const solHolding = dashboard?.tokenRows.find(
      (tokenHolding) => tokenHolding.symbol === 'SOL',
    );

    expect(solHolding).toBeDefined();
    if (!dashboard || !solHolding) {
      throw new Error('expected dashboard SOL holding to be defined');
    }

    await extensionService.handleRequest('helio/update-auto-yield-settings', {
      settings: {
        ...dashboard.autoYield.settings,
    ...
    expect(
      refreshedDashboard.autoYield.reserve.totalDeployedUsd,
    ).toBeGreaterThan(0);
