...
async function withSessionKeypair<T>(
  fn: (keypair: Keypair) => Promise<T> | T,
): Promise<T> {
  const secret = await readSessionSecret();
  if (!secret) {
    throw new HelioCoreError(
      'Unlock Helio to approve this request.',
      'SESSION_LOCKED',
    );
  }
...
async function addTrustedOrigin(origin: string): Promise<void> {
  ...
  if (!origins.includes(origin) && hasChromeSession()) {
    await chrome.storage.session.set({
      [TRUSTED_ORIGINS_KEY]: [...origins, origin],
    });
  }
}
...
function normalizeOrigin(origin: string): string {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new HelioCoreError(
      'Only http(s) dApp origins are supported.',
      'INVALID_DAPP_ORIGIN',
    );
  }
...
function messagePreview(messageBase64: string): string {
  try {
    ...
    const bytes =
      allBytes.length > MAX_PREVIEW_BYTES
        ? allBytes.slice(0, MAX_PREVIEW_BYTES)
        : allBytes;
    ...
    return readable
      ? text.slice(0, 280)
      : `[Binary message: ${allBytes.length} bytes]`;
  } catch {
...
async function handleConnect(
  request: ConnectDappRequest,
): Promise<DappConnectionState> {
  const origin = normalizeOrigin(request.origin);
  const publicKey = await sessionPublicKey();
  if (!publicKey)
    throw new HelioCoreError(
      'Unlock Helio to connect this site.',
      'SESSION_LOCKED',
    );
  ...
  const risk = await riskProvider.assessConnection({
...
async function handleSignTransaction(
  request: RequestDappTransactionSignatureRequest,
): Promise<never> {
  const origin = normalizeOrigin(request.origin);
  if (!(await sessionPublicKey()))
    throw new HelioCoreError('Unlock Helio to sign.', 'SESSION_LOCKED');
  ...
  pendingDappRequest = {
    ...
    review: {
      ...
      dapp,
      summaryLines: [
        'This site is requesting your signature on a transaction.',
      ],
      warnings: risk.warnings,
      sendReview: null,
    },
...
async function handleSignMessage(
  request: RequestDappMessageSignatureRequest,
): Promise<never> {
  const origin = normalizeOrigin(request.origin);
  if (!(await sessionPublicKey()))
    throw new HelioCoreError('Unlock Helio to sign.', 'SESSION_LOCKED');
...
async function handleApprove(id: string): Promise<DappApprovedRequestResult> {
  ...
  const signed = await withSessionKeypair(async (keypair) => {
    ...
    const signature = await signMessageWithSecretKey(
      decodeBase64(request.messageBase64),
      keypair.secretKey,
    );
...
async function handleConnectionState(
  ...
  return {
    origin,
    isConnected: connected && publicKey !== null,
    account: publicKey ? accountSummary(publicKey) : null,
  };
}
...
export function createDappHandler(): DappHandler {
  return {
    ...
    isDecision: (type) =>
      type === 'helio/approve-dapp-request' ||
      type === 'helio/reject-dapp-request',
      ...
      switch (type) {
        ...
        case 'helio/sign-dapp-transaction':
          return handleSignTransaction(
            payload as RequestDappTransactionSignatureRequest,
          );
        case 'helio/sign-dapp-message':
          return handleSignMessage(
            payload as RequestDappMessageSignatureRequest,
          );
